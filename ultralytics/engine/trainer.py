...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        # AFSS: Initialize Anti-Forgetting Sampling Strategy if enabled
        if getattr(self.args, "afss", False) and RANK in {-1, 0}:
            ...
            if getattr(self.args, "afss_auto_tune", False):
                # Get number of classes from dataset or model
                nc = 10
                if hasattr(self.train_loader.dataset, "data") and isinstance(self.train_loader.dataset.data, dict):
                    nc = self.train_loader.dataset.data.get("nc", nc)
                elif hasattr(self, "model") and self.model is not None:
                    nc = getattr(self.model, "nc", nc)

                tuned = suggest_afss_params(
                    num_images=n_train_images,
                    batch_size=batch_size,
                    epochs=self.epochs,
                    num_classes=nc,
                    easy_thresh=getattr(self.args, "afss_easy_thresh", 0.8),
                    hard_thresh=getattr(self.args, "afss_hard_thresh", 0.3),
                )
                if not tuned.get("afss", True):
                    LOGGER.warning("AFSS auto-tune: disabling AFSS (dataset too small).")
                    self.afss_manager = None
                else:
                    # Override args with tuned values
                    easy_thresh = tuned["afss_easy_thresh"]
                    hard_thresh = tuned["afss_hard_thresh"]
                    easy_ratio = tuned["afss_easy_ratio"]
                    moderate_ratio = tuned["afss_moderate_ratio"]
                    update_interval = tuned["afss_update_interval"]
                    warmup_epochs = tuned["afss_warmup_epochs"]
            else:
                # Manual mode: use user-provided or default values
                easy_thresh = getattr(self.args, "afss_easy_thresh", 0.8)
                hard_thresh = getattr(self.args, "afss_hard_thresh", 0.3)
                easy_ratio = getattr(self.args, "afss_easy_ratio", 0.02)
                moderate_ratio = getattr(self.args, "afss_moderate_ratio", 0.4)
                update_interval = getattr(self.args, "afss_update_interval", 5)
                warmup_epochs = getattr(self.args, "afss_warmup_epochs", 10)

            if self.afss_manager is None and getattr(self.args, "afss_auto_tune", False):
                pass  # auto-tune decided dataset is too small, skip AFSS
            else:
                ...
                afss_batch_sampler = AFSSBatchSampler(
                    afss_manager=self.afss_manager,
                    batch_size=batch_size,
                    drop_last=False,
                )
                # Rebuild train_loader with AFSS batch sampler
                self.train_loader = _build_dataloader(
                    ...
                    shuffle=False,
                    rank=LOCAL_RANK,
                    batch_sampler=afss_batch_sampler,
                )
                LOGGER.info(
                    f"AFSS enabled: {n_train_images} images, "
                    f"easy_thresh={self.afss_manager.easy_thresh}, "
                    f"hard_thresh={self.afss_manager.hard_thresh}, "
                    f"easy_ratio={self.afss_manager.easy_ratio}, "
                    f"moderate_ratio={self.afss_manager.moderate_ratio}, "
                    f"update every {self.afss_manager.update_interval} epochs, "
                    f"warmup {self.afss_manager.warmup_epochs} epochs"
                )
    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            if RANK in {-1, 0}:
                ...
                # AFSS: Update per-image learning sufficiency
                    ...
                    if self.afss_manager.should_update(epoch + 1):
                        ...
                        if adapt_info:
                            LOGGER.info(
                                ...
                                f"(Easy {adapt_info['n_easy']}, Moderate {adapt_info['n_moderate']}, "
                                f"Hard {adapt_info['n_hard']})\n"
                                f"  easy_ratio: {adapt_info['old_easy_ratio']:.3f} → {adapt_info['new_easy_ratio']:.3f}\n"
                                f"  moderate_ratio: {adapt_info['old_moderate_ratio']:.3f} → {adapt_info['new_moderate_ratio']:.3f}\n"
                                f"  active set: {adapt_info['active']:,} / {self.afss_manager.num_images:,} "
                                f"({adapt_info['active_ratio']:.0%})"
                            )
