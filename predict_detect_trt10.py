...
class YOLOv8TRT:
    ...
    def _allocate_buffers(self):
        """为所有输入和输出张量分配缓冲区 (TensorRT 10.8 Tensor API)"""
        self.inputs = []
        self.outputs = []
        self._allocated_buffers = []  # 重置已分配缓冲区列表

        n_io_tensors = self.engine.num_io_tensors
        tensor_names = [self.engine.get_tensor_name(i) for i in range(n_io_tensors)]

        for name in tensor_names:
            tensor_mode = self.engine.get_tensor_mode(name)
            trt_dtype = self.engine.get_tensor_dtype(name)
            dtype_np = trt.nptype(trt_dtype)  # 例如 np.float32
            element_size = np.dtype(dtype_np).itemsize  # 关键修正：获取具体dtype对象的itemsize
            shape = self.engine.get_tensor_shape(name)

            if -1 in shape:
                num_elements = 1
                print(f"Warning: Tensor {name} has dynamic shape: {shape}. Initial allocation is minimal.")
            else:
                num_elements = trt.volume(shape)

            size_in_bytes = num_elements * element_size

            device_mem = cuda.mem_alloc(size_in_bytes)
            self._allocated_buffers.append(device_mem)  # 跟踪分配的内存

            self.context.set_tensor_address(name, int(device_mem))

            # 构建 tensor_info 字典，确保包含 'dtype' 键
            tensor_info = {
                'name': name,
                'shape': shape,
                'trt_dtype': trt_dtype,
                'dtype': dtype_np,  # 确保这个键存在，其值是NumPy数据类型，例如np.float32
    ...
    def inference(self, blob):
        """执行模型推理 (TensorRT 10.8 Tensor API)"""
        if self.engine is None:
            raise ValueError("模型未加载，请先调用 load_model 方法")

        # 检查是否有输入张量
        if not self.inputs:
            raise RuntimeError("No input tensors found. Buffer allocation might have failed.")
        input_tensor_info = self.inputs[0]
        input_name = input_tensor_info['name']

        # 处理动态输入形状
        if -1 in input_tensor_info['shape']:
            actual_input_shape = blob.shape
            self.context.set_input_shape(input_name, actual_input_shape)
            print(f"Debug: Set input shape to {actual_input_shape} for tensor '{input_name}'")

        # 将输入数据复制到设备
        cuda.memcpy_htod_async(input_tensor_info['device_mem'], blob, self.stream)
        ...
        for out_info in self.outputs:
            # 获取实际的输出形状（考虑动态形状后）
            actual_output_shape = self.context.get_tensor_shape(out_info['name'])
            # 确保 out_info 字典中有 'dtype' 键
            if 'dtype' not in out_info:
                # 应急处理：尝试使用其他方式获取dtype，但最好在_allocate_buffers中修复
                # 例如，如果存了'trt_dtype'，可以转换：dtype_np = trt.nptype(out_info['trt_dtype'])
                # 这里为了演示，假设应该是np.float32，但这不是长久之计
                print(f"Warning: 'dtype' key missing in out_info for {out_info['name']}. Using np.float32 as fallback.")
                output_dtype = np.float32
            else:
                output_dtype = out_info['dtype']  # 正常从字典中获取

            # 根据实际形状创建主机内存
            host_mem = np.empty(actual_output_shape, dtype=output_dtype)  # 使用确定的dtype
